...
function ReservationsAmountChartCard() {
  ...
  const fetchAmount = async () => {
    ...
    try {
      const [sum, avg] = await Promise.all([
        AnalyticsAPI.getReservationsAmount(
          dateRange.from,
          dateRange.to,
          false,
          granularity
      ...
      const sumAndAvgData: SumAndAverageAreaChartData[] = sum.map(
        (sumEntry) => {
          ...
          return {
            ...
            sum: sumEntry.reservationsTotalAmount,
            average: avgEntry ? avgEntry.reservationsTotalAmount : 0,
          };
        }
      );
  ...
  return (
    <AreaChartCard
      title={"Reservations Earnings"}
      subtitle="Reservation's average and total amounts"
      ...
      yAxeUnit="EURO"></AreaChartCard>
  );
}

export default ReservationsAmountChartCard;
